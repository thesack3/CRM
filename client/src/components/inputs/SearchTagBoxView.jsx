  ...
  // });
    ...
    useEffect(() => {
      ...
      if(props.Lead){
        ...
        const tagIds = props.Lead.Tags;
  
        for (let i = 0; i < tagIds.length + 1 ; i += 1) {
          const tagObject = tagsData.tags.find((tag) => tag.id === tagIds[i]);
          if (tagObject) {
      
            setRetreivedTags((prevState) => [...prevState, tagObject]);

            // Refresh and set the tag objects array to a mapping of tags within the listbox. 

            console.log("tagObject");
            console.log(tagObject);
            console.log(tagObject.title);
